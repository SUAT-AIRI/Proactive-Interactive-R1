...
from verl.utils.reward_score import math_ori


def char_count_reward_function(data_source, solution_str, ground_truth, extra_info=None):
    try:
        last_boxed_string = math_ori.last_boxed_only_string(solution_str)
        if last_boxed_string is None:
            return 0
        solution = math_ori.remove_boxed(last_boxed_string)
